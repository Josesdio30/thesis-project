...
export default function AssignmentTab() {
        ...
        a.id === selectedAssignment.id
          ? {
              ...
              answers: a.answers.map(ans =>
                ans.student === userName ? { ...ans, answers: answer } : ans
              ),
              done: true,
            }
          : a
  ...
  const handleOptionChange = (qIdx: number, oIdx: number, value: string) => {
    setForm(f => ({
      ...
      questions: f.questions.map((q, i) =>
        i === qIdx
          ? { ...q, options: q.options.map((opt: string, j: number) => (j === oIdx ? value : opt)) }
          : q
      ),
    }));
  };
  ...
  return (
    <div className="relative">
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
        {assignments.map((a) => (
          <div
            ...
            className={`relative bg-white p-6 rounded-md shadow border transition hover:shadow-lg group cursor-pointer ${a.done ? 'opacity-80' : ''}`}
          >
      ...
      <Modal open={showAddModal} onClose={() => setShowAddModal(false)}>
            ...
            {q.type === 'pilihan_ganda' && (
              <div className="mb-2">
                ...
                <button
                  ...
                  onClick={e => { e.preventDefault(); handleAddOption(idx); }}
                >
                  + Tambah Opsi
                </button>
              </div>
        ...
        <button
          className="bg-gray-200 text-gray-700 px-3 py-1 rounded mb-2 mr-2"
                        ...
                        return (
                          <div className="bg-gray-100 p-2 rounded mb-2">
                            ...
                            <div className="text-gray-800">
                              {myAns.answers[idx] ? myAns.answers[idx] : <span className="italic text-gray-400">Belum dijawab</span>}
                            </div>
                          </div>
                        );
                      } else {
                      ...
                      <div>
                        {selectedAssignment.questions.map((q: any, qIdx: number) => (
                          <div key={qIdx} className="mb-1">
                            <span className="font-semibold">{qIdx + 1}.</span> {ans.answers[qIdx] || <span className="italic text-gray-400">Belum dijawab</span>}
                          </div>
                        ))}
                      </div>
                      <div>
